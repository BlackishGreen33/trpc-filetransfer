...
type TestStreamService interface {
	...
	DownloadFileStream(*DownloadFileReq, TestStream_DownloadFileStreamServer) error
}
...
func TestStreamService_DownloadFileStream_Handler(srv interface{}, stream server.Stream) error {
	m := new(DownloadFileReq)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(TestStreamService).DownloadFileStream(m, &testStreamDownloadFileStreamServer{stream})
}

type TestStream_DownloadFileStreamServer interface {
	Send(*DownloadFileResp) error
	server.Stream
}

type testStreamDownloadFileStreamServer struct {
	server.Stream
}

func (x *testStreamDownloadFileStreamServer) Send(m *DownloadFileResp) error {
	return x.Stream.SendMsg(m)
}
...
var TestStreamServer_ServiceDesc = server.ServiceDesc{
	...
	Streams: []server.StreamDesc{
		...
		{
			StreamName:    "/trpc.examples.stream.TestStream/DownloadFileStream",
			Handler:       TestStreamService_DownloadFileStream_Handler,
			ServerStreams: true,
		},
	},
}
...
func (s *UnimplementedTestStream) DownloadFileStream(req *DownloadFileReq, stream TestStream_DownloadFileStreamServer) error {
	return errors.New("rpc DownloadFileStream of service TestStream is not implemented")
}
...
type TestStreamClientProxy interface {
	...
	DownloadFileStream(ctx context.Context, req *DownloadFileReq, opts ...client.Option) (TestStream_DownloadFileStreamClient, error)
}
...
func (c *TestStreamClientProxyImpl) DownloadFileStream(ctx context.Context, req *DownloadFileReq, opts ...client.Option) (TestStream_DownloadFileStreamClient, error) {
	ctx, msg := codec.WithCloneMessage(ctx)

	msg.WithClientRPCName("/trpc.examples.stream.TestStream/DownloadFileStream")
	msg.WithCalleeServiceName(TestStreamServer_ServiceDesc.ServiceName)
	msg.WithCalleeApp("examples")
	msg.WithCalleeServer("stream")
	msg.WithCalleeService("TestStream")
	msg.WithCalleeMethod("DownloadFileStream")
	msg.WithSerializationType(codec.SerializationTypePB)

	clientStreamDesc := &client.ClientStreamDesc{}
	clientStreamDesc.StreamName = "/trpc.examples.stream.TestStream/DownloadFileStream"
	...
	stream, err := c.streamClient.NewStream(ctx, clientStreamDesc, "/trpc.examples.stream.TestStream/DownloadFileStream", callopts...)
	if err != nil {
		return nil, err
	}
	x := &testStreamDownloadFileStreamClient{stream}
...
type TestStream_DownloadFileStreamClient interface {
	Recv() (*DownloadFileResp, error)
	client.ClientStream
}

type testStreamDownloadFileStreamClient struct {
	client.ClientStream
}

func (x *testStreamDownloadFileStreamClient) Recv() (*DownloadFileResp, error) {
